    ...
    public static function signup($name, $Email, $teleNo, $address, $NIC, $password)
    {
        if ($name && $Email && $teleNo && $address && $NIC && $password){
            ...
            if($result){
                ...
                $sql = "INSERT INTO passenger(accName,name,tel,address,password,dp,NIC,pin_code) VALUES(?, ?, ?, ?, ?, ?, ?, ?)";
                $stmt = (new Connection)->connect()->prepare($sql);
                if($stmt->execute([$Email, $name, $teleNo, $address, $password, NULL, $NIC,rand(00000000,99999999)])){
